...
internal class ConductBoardManager : MonoBehaviour
{
    ...
    private async void CreateEntries()
    {
        ...
        foreach (KeyValuePair<string, BepInEx.PluginInfo> info in Chainloader.PluginInfos)
        {
            ...
            {
                if (string.IsNullOrEmpty(attribute.Title) || string.IsNullOrWhiteSpace(attribute.Title) ||
                    string.IsNullOrEmpty(attribute.Text)  || string.IsNullOrWhiteSpace(attribute.Text)) continue;
    ...
    private async Task DownloadEntries()
    {
    ...
    {
        public readonly bool UseBaseText;

        [TextArea(1, 1)] public readonly string Title;

        [TextArea(12, 32)] public readonly string Body;

        public Section() => UseBaseText = true;
