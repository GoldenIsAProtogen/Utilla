...
internal class UtillaGamemodeSelector : MonoBehaviour
{
    ...
    public void OnSelectorSetup()
    {
        ...
        SelectorGameModes.Remove(sessionIsPrivate);
    ...
    public List<Gamemode> GetSelectorGameModes()
    {
        ...
        GameModeType[] modesForZone = Layout is CustomMapModeSelector
                                              ? [.. CustomMapModeSelector.gamemodes,]
                                              :
                                              [
                                                      .. GameMode.GameModeZoneMapping.GetModesForZone(Zone,
                                                              NetworkSystem.Instance.SessionIsPrivate),
                                              ];

        // Base gamemodes
        foreach (GameModeType t in modesForZone)
            if (GamemodeManager.Instance.DefaultGameModesPerMode.TryGetValue(t, out Gamemode gamemode))
                gameModeList.Add(gamemode);

        // Modded gamemodes
        foreach (GameModeType t in modesForZone)
        {
            if (GamemodeManager.Instance.ModdedGamemodesPerMode.TryGetValue(t, out Gamemode gamemode))
            {
                Logging.Info($"+ \"{gamemode.DisplayName}\" ({t.GetName()})");
                gameModeList.Add(gamemode);
        ...
        // Custom gamemodes
        if (GamemodeManager.Instance.CustomGameModes is { } customGameModes)
            foreach (Gamemode gameMode in customGameModes)
            {
        ...
        List<Gamemode> finalGameModeList =
        [
                .. superModeState
                           ? gameModeList.Where(gameMode =>
                                                        !gameMode.BaseGamemode.HasValue ||
                                                        gameMode.BaseGamemode.Value !=
                                                        GameModeType.Casual &&
                                                        gameMode.BaseGamemode.Value !=
                           ...
                           : gameModeList.Where(gameMode =>
                                                        !gameMode.BaseGamemode.HasValue ||
                                                        gameMode.BaseGamemode.Value !=
                                                        GameModeType.SuperCasual &&
                                                        gameMode.BaseGamemode.Value !=
                                                        GameModeType.SuperInfect),
        ];

        if (SelectorGameModes.TryAdd(sessionIsPrivate, finalGameModeList))
            Logging.Info(string.Join(", ",
                    finalGameModeList.Where(gameMode => gameMode != null).Select(gameMode => gameMode.DisplayName)
                                     .Select(gameMode => $"\"{gameMode}\"")));
    ...
    public void CheckGameMode()
    {
        ...
        string[] modeNames = GetSelectorGameModes().Where(mode => mode != null).Select(mode => mode.ID).ToArray();
        int      index     = Array.IndexOf(modeNames, currentMode);

        if (index == -1)
        {
            if (modeNames.Length > 0) SetDefaultMode(modeNames.First());
    ...
    public void SetDefaultMode(string defaultMode)
    {
        ...
        if (isModded)
        {
            ...
            if (isBaseMode &&
                GamemodeManager.Instance.ModdedGamemodesPerMode.TryGetValue(modeType, out Gamemode moddedGamemode))
                defaultMode = moddedGamemode.ID;
        }

        GorillaComputer.instance.SetGameModeWithoutButton(defaultMode);

        string[] modeNames = GetSelectorGameModes().Select(game_mode => game_mode != null ? game_mode.ID : string.Empty)
                                                   .ToArray();
    ...
    {
        CurrentPage = PageCount <= 0 ? 0 : CurrentPage <= 0 ? PageCount - 1 : CurrentPage - 1;
        ShowPage();
    }
    ...
    public void ShowPage()
    {
        ...
        PageCount   = Mathf.Max(1, Mathf.CeilToInt(allGamemodes.Count / (float)Mathf.Max(PageCapacity, 1)));
    ...
    private void CreatePageButtons(GameObject templateButton)
    {
        ...
        {
            GameObject button =
                    Instantiate(templateButton.transform.childCount == 0 ? fallbackTemplateButton : templateButton, templateButton.transform.parent, true);
            ...
            if (button.transform.Find("NewSplash") is { } splash && splash && splash.gameObject.activeSelf)
        ...
        nextPageButton.transform.localPosition = new Vector3(isCustomSelector ? -0.78f : -0.745f,
                isCustomSelector ? 0.1f : -0.095f, -0.03f);
    ...
    private void ModifySelectionButtons()
    {
        ...
        foreach (ModeSelectButton mb in Layout.currentButtons.Where(mb => !moddedSelectionButtons.Contains(mb)))
        {
