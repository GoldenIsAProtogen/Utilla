...
internal static class GorillaComputerPatches
{
    ...
    [HarmonyPatch(nameof(GorillaComputer.SetGameModeWithoutButton))]
    [HarmonyPrefix]
    ...
    public static bool SetModePatch(string gameMode, MethodBase __originalMethod)
    {
        ...
        if (frame?.GetMethod() is MethodBase methodBase &&
            methodBase.DeclaringType.FullName.Contains("GameModeSelectorButtonLayout")) return false;

        return AllowSettingMode;
    }

    [HarmonyPatch(nameof(GorillaComputer.UpdateGameModeText))]
    [HarmonyPrefix]
    ...
    private static IEnumerable<CodeInstruction> InitializeModeTranspiler(IEnumerable<CodeInstruction> instructions)
    {
        ...
        for (int i = 0; i < codes.Length; i++)
            if (codes[i].opcode == OpCodes.Call)
            {
                ...
                if (methodName    != "GetString" || returnType != typeof(string) ||
                    declaringType != typeof(PlayerPrefs)) continue;
